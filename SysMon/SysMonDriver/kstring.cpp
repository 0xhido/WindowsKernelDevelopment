...
kstring::kstring(PCUNICODE_STRING str, POOL_TYPE pool, ULONG tag) : m_Pool(pool), m_Tag(tag) {
	...
	m_Capacity = m_Len + 1;
	m_str = Allocate(m_Capacity, str->Buffer);
	if (!m_str)
		ExRaiseStatus(STATUS_NO_MEMORY);
}
...
kstring& kstring::operator+=(PCWSTR str) {
	...
	wchar_t* newBuffer = Allocate(m_Len, m_str);
	ULONG bytes = m_Len + 1;
	wcscat_s(newBuffer, bytes, str);
...
PUNICODE_STRING kstring::GetUnicodeString(PUNICODE_STRING pUnicodeString) {
	RtlInitUnicodeString(pUnicodeString, m_str);
	return pUnicodeString;
}

wchar_t* kstring::Allocate(size_t chars, const wchar_t* src) {
	wchar_t* str = static_cast<wchar_t*>(ExAllocatePoolWithTag(m_Pool, sizeof(WCHAR) * (chars + 1), m_Tag));
	if (!str) {
		KdPrint(("Failed to allocate kstring of length %I64u chars\n", chars));
		return nullptr;
	}
	if (src)
